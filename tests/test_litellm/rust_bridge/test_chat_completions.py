...
def _should_attempt(**overrides) -> bool:
    ...
    return bridge.should_attempt_rust_chat_completions(**kwargs)
...
class TestEligibility:
    ...
    def test_checks_the_selected_sync_or_async_callable(self, monkeypatch):
        _hide_native_bridge(monkeypatch)
        bridge.set_rust_chat_completions(achat_completions=_RecordingAsyncCall())
        assert _should_attempt(asynchronous=False) is False
        assert _should_attempt(asynchronous=True) is True
