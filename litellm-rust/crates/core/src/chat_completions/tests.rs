...
async fn an_unsupported_request_is_typed_as_not_sent() {
    ...
    assert!(matches!(
        error,
        Error::NotSent(ref inner)
            if matches!(inner.as_ref(), Error::Unsupported("streaming"))
    ));
}
...
mod round_trip {
    ...
    async fn a_response_it_cannot_normalize_is_reported_as_already_sent() {
        ...
        assert!(
            matches!(
                err,
                Error::PossiblySent(ref inner)
                    if matches!(inner.as_ref(), Error::InvalidResponse(_))
            ),
    ...
    async fn an_upstream_error_status_keeps_its_code() {
        ...
        assert!(
            matches!(
                err,
                Error::PossiblySent(ref inner)
                    if matches!(inner.as_ref(), Error::Http { status: 429, .. })
            ),
    ...
    async fn a_connection_that_is_never_established_declines_instead_of_failing() {
        ...
        assert!(
            matches!(
                err,
                Error::NotSent(ref inner) if matches!(inner.as_ref(), Error::Connect(_))
            ),
