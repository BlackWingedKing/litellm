...
async fn messages_requires_auth_when_no_key_and_no_header() {
    ...
    assert!(matches!(
        err,
        Error::NotSent(ref inner) if matches!(inner.as_ref(), Error::Auth(_))
    ));
}
...
async fn messages_maps_provider_error_status_to_http_error() {
    ...
    assert!(matches!(
        ...
        Error::PossiblySent(ref inner)
            if matches!(inner.as_ref(), Error::Http { status: 401, .. })
    ));
}
...
async fn messages_rejects_unsupported_provider() {
    ...
    assert!(matches!(
        ...
        Error::NotSent(ref inner)
            if matches!(inner.as_ref(), Error::InvalidProvider(provider) if provider == "openai")
    ));
}
