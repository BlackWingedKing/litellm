    ...
    def completion(
        ...
        def sync_python_path(*, skip_pre_call_logging: bool) -> ModelResponse | CustomStreamWrapper:
            ...
            request_client: Final = (
                _get_httpx_client(
                    {"timeout": request_timeout}  # mutable-ok: legacy client requires a dict
                    if request_timeout is not None
                    else {}  # mutable-ok: legacy client requires a dict
                )
        ...
        # whether to attempt it before transforming so pre_call fires once, and
