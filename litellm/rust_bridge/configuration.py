...
def env_enables_rust_ocr() -> bool:
    global_override: Final = _parse_env_bool(_GLOBAL_ENV_NAME, os.getenv(_GLOBAL_ENV_NAME))
    if global_override is not None:
        return global_override
    legacy_override: Final = _parse_env_bool(_LEGACY_OCR_ENV_NAME, os.getenv(_LEGACY_OCR_ENV_NAME))
    return legacy_override is True
...
def use_litellm_rust(
    ...
    *,
    ...
    configuring_bindings: Final = any(
        not isinstance(binding, Unset)
        for binding in (ocr, aocr, messages, amessages, responses_websocket, transcription, atranscription)
    )
    if not configuring_bindings:
        return
    warnings.warn(
    ...
    set_rust_ocr(ocr=ocr, aocr=aocr)
    set_rust_messages(messages=messages, amessages=amessages)
    configure_rust_transcription(transcription=transcription, atranscription=atranscription)
